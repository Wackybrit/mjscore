...
<form method="POST" action="/new-game">
    ...
    ${[0, 1, 2, 3].map(index => `
        <div>
            <label>
                Player ${index + 1} Name:
                <input type="text" name="playerName_${index}" required
                ${index === 0 ? "autofocus" : ""}>
            </label>
...
<script>
    const windSelects = Array.from(
        document.querySelectorAll(".wind-select")
    );

    function updateWindOptions() {
        const selectedWinds = windSelects
        ...
        windSelects.forEach(select => {
            ...
            Array.from(select.options).forEach(option => {
                ...
                option.disabled =
                    selectedWinds.includes(option.value) &&
...
`
    ...
    app.post("/new-game", (req, res) => {
        ...
        const selectedWinds = entries.map(entry => entry.startingWind);
        ...
        const sortedPlayerNames = WIND_ORDER.map(wind => {
